...
class PlayState extends MusicBeatState
{
	...
	function hittingSpaceSHit()
	{
		...
		else if (pressedSpaceCounter > tolerance)
		{
			doDamage = true;
		}
	}
	...
	override public function update(elapsed:Float)
	{
		...
		{
			camFollow.x = (baseX + musthitaddX);
			camFollow.y = (baseY + musthitaddY);
		}
		...
		iconP1.y = ((healthBar.height * (FlxMath.remapToRange(healthBar.percent, 0, 100, 100, 0) * 0.01))
			+ (150 * iconP1.scale.y - 150) / 2
			- iconOffset)
			+ adj;
		iconP2.y = ((healthBar.height * (FlxMath.remapToRange(healthBar.percent, 0, 100, 100, 0) * 0.01))
			- (150 * iconP2.scale.y) / 2
			- iconOffset
			- 22 * 2)
			+ adj;
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String)
	{
		switch (eventName)
		{
			...
			case 'FinalClash':
				...
				FlxTween.tween(beysglow, {alpha: 1}, 0.8, {
					onComplete: function(twn:FlxTween)
					{
						FlxTween.tween(beysglow, {alpha: 0}, 0.000001); ////oh my fucking GOD
						new FlxTimer().start(timeClash, function(tmr:FlxTimer)
						{
							camGame.shake(0.0093, 0.1);
						});
			...
			case 'lightShow':
				...
				{
					value1 = "";
				} // cuz i keep making these with whitespaces
				if (value1 == "")
				{
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if (!note.noAnimation)
			{
				...
				{
					case 'singLEFT':
						camFollow.x = (baseX + musthitaddX) - camChange;
						camFollow.y = (baseY + musthitaddY);
					case 'singRIGHT':
						camFollow.x = (baseX + musthitaddX) + camChange;
						camFollow.y = (baseY + musthitaddY);
					case 'singUP':
						camFollow.y = (baseY + musthitaddY) - camChange;
						camFollow.x = (baseX + musthitaddX);
					case 'singDOWN':
						camFollow.y = (baseY + musthitaddY) + camChange;
						camFollow.x = (baseX + musthitaddX);
	...
	var parryEvents:Array<Int> = [
		352, 740, 1016, 1264, 1864, 2064, 2384, 2640, 3160, 3408, 3696, 3760, 3824, 3888, 4076, 4592, 4960, 5152, 5505, 5632, 6192, 6304, 6684, 6944, 7136,
		7648,
		7968, 8352, 8608, 9008, 9344, 9408, 9472, 9648, 9984, 10368, 10688, 10816, 10960, 11568, 11984, 12256, 12528, 12880, 13008, 13744, 14128, 14408, 14792,
		15040, 15328, 99999
	];
