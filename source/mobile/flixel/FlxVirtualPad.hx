...
class FlxVirtualPad extends FlxSpriteGroup
{
	...
	private function createButton(X:Float, Y:Float, Graphic:String, Color:Int = 0xFFFFFF):FlxButton
	{
		...
		if (Assets.exists(Paths.image('virtualpad/${Graphic}', 'shared')))
			graphic = FlxG.bitmap.add(Paths.image('virtualpad/${Graphic}', 'shared'));
		else
			graphic = FlxG.bitmap.add(Paths.image('virtualpad/default', 'shared'));
